  ...
  ${sidebarOpen ? 'translate-x-0' : 'max-sm:-translate-x-full'} transition-all duration-300 ease-in-out`}>
        <div className='w-full'>
          <div className='flex items-center justify-between px-7 py-2'>
            <img onClick={()=>navigate('/')} src={assets.logo} className='w-26 cursor-pointer' alt="" />
            <NotificationBell />
          </div>
        ...
        <div className='w-full border-t border-gray-200 p-4 px-7 flex items-center justify-between'>
          <div className='flex gap-2 items-center cursor-pointer' onClick={() => navigate('/profile')}>
              <img
                ...
                onError={(e) => { e.target.src = `https://ui-avatars.com/api/?name=${encodeURIComponent(user?.full_name || 'U')}&background=6366f1&color=fff` }}
              />
              <div>
          ...
          <LogOut onClick={handleLogout} className='w-4 text-gray-400 hover:text-gray-700 transition cursor-pointer'/>
        </div>
    </div>  
  )
}
